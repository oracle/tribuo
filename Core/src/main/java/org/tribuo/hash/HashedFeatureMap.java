...
public final class HashedFeatureMap extends ImmutableFeatureMap {
     ...
     */
    public static HashedFeatureMap deserializeFromProto(int version, String className, Any message, ProtoDeserializationCache deserCache) throws InvalidProtocolBufferException {
        ...
        Hasher hasher = ProtoUtil.deserialize(hasherProto, deserCache);
        HashedFeatureMap obj = new HashedFeatureMap(hasher);
        for (VariableInfoProto infoProto : proto.getInfoList()) {
            VariableIDInfo info = ProtoUtil.deserialize(infoProto, deserCache);
