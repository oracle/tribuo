...
public class Util {
     ...
     */
    public static void shuffleInPlace(SparseVector[] features, SparseVector[] labels, double[] weights, SplittableRandom rng) {
        ...
        for (int i = size; i > 1; i--) {
            ...
            //swap labels
            SparseVector tmpLabel = labels[i-1];
            labels[i-1] = labels[j];
            labels[j] = tmpLabel;
            //swap weights
     ...
     * @param rng SplittableRandom number generator.
     */
    public static void shuffleInPlace(SparseVector[] features, SparseVector[] labels, double[] weights, int[] indices, SplittableRandom rng) {
        int size = features.length;
        // Shuffle array
        for (int i = size; i > 1; i--) {
            int j = rng.nextInt(i);
     ...
     */
    public static ExampleArray shuffle(SparseVector[] features, SparseVector[] labels, double[] weights, SplittableRandom rng) {
        int size = features.length;
        SparseVector[] newFeatures = new SparseVector[size];
        SparseVector[] newLabels = new SparseVector[size];
        ...
        for (int i = size; i > 1; i--) {
            ...
            //swap labels
            SparseVector tmpLabel = newLabels[i-1];
     ...
     */
    static class ExampleArray {
        public final SparseVector[] features;
        public final SparseVector[] labels;
        public final double[] weights;

        ExampleArray(SparseVector[] features, SparseVector[] labels, double[] weights) {
