...
training algorithm before producing the `Model`. A Model contains the necessary
...
` interface allows for a `postConfig` method, which is called after the
 ...
 `postConfig` method is used to perform the validation that would normally be
  ...
  about OLCUT can be found in it's 
...
indices). The predicate function accepts an `Example` and thus can depend on
...
provenance. Each type of provenance can be removed separately. It is also
 ...
 model as a tracking mechanism. We intend for the user to store the
